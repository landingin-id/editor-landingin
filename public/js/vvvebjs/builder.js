...
Vvveb.Builder = {
	...
	_frameLoaded: function () {
		...
		// Tambahkan event listener untuk zoom dengan mouse wheel pada canvas
		self._initCanvasZoom();
		
		// Tambahkan event listener untuk drag/pan canvas
		self._initCanvasDrag();
	},

	_initCanvasZoom: function () {
		let self = Vvveb.Builder;
		...
		// Fungsi zoom yang akan digunakan oleh semua event listener
		function handleZoom(e) {
			// Hanya aktifkan zoom ketika tombol Command ditekan
			if (!e.metaKey) return;
			...
			// Dapatkan nilai zoom saat ini
			let currentZoom = zoomInput ? parseInt(zoomInput.value) || 100 : 100;
			
			// Tentukan arah zoom (zoom in atau zoom out)
			let zoomDelta = e.deltaY > 0 ? -10 : 10; // Scroll down = zoom out, scroll up = zoom in
			
			// Hitung zoom baru
			let newZoom = currentZoom + zoomDelta;
			
			// Terapkan zoom baru
			Vvveb.Gui.setZoom(newZoom);
		}
		
		// Tambahkan event listener pada canvas
		if (canvas) {
			canvas.addEventListener("wheel", handleZoom, { passive: false });
		}
		
		// Tambahkan event listener pada iframe-wrapper
		if (iframeWrapper) {
			iframeWrapper.addEventListener("wheel", handleZoom, { passive: false });
		}
		
		// Tambahkan event listener pada iframe setelah dimuat
		if (iframe) {
			iframe.addEventListener("load", function() {
				try {
					// Tambahkan event listener pada document iframe
					if (iframe.contentDocument) {
						iframe.contentDocument.addEventListener("wheel", handleZoom, { passive: false });
					}
				} catch (e) {
			...
			// Jika iframe sudah dimuat, tambahkan event listener sekarang
			try {
				if (iframe.contentDocument) {
					iframe.contentDocument.addEventListener("wheel", handleZoom, { passive: false });
				}
		...
		// Tambahkan fitur drag untuk menggeser canvas
		self._initCanvasDrag();
	},

	_initCanvasDrag: function () {
		let canvas = document.getElementById("canvas");
		...
		function handleMouseDown(e) {
			...
			// Check if click is within canvas or iframe area
			let canvasRect = canvas.getBoundingClientRect();
			let isInCanvas = e.clientX >= canvasRect.left && e.clientX <= canvasRect.right &&
		...
		function handleMouseMove(e) {
			if (isDragging && cmdPressed) {
				...
				wrapper.style.transform = `translate(${currentTransformX}px, ${currentTransformY}px)`;
		...
		// Event listeners - attach to document to capture all mouse events
...
Vvveb.Gui = {
	...
	zoomChange: function () {
		...
		// Update zoom input value
		let zoomInput = document.getElementById("zoom");
		if (zoomInput && this.value) {
			zoomInput.value = this.value;
		}
	},

	// Fungsi untuk mengatur zoom dengan nilai tertentu
	setZoom: function (zoomValue) {
		let wrapper = document.getElementById("iframe-wrapper");
		let zoomInput = document.getElementById("zoom");
		
		// Batasi zoom antara 10% dan 200%
		zoomValue = Math.max(10, Math.min(200, zoomValue));
		
		let scale = "";
		let height = "";
		if (zoomValue != 100) {
			scale = "scale(" + zoomValue + "%)";
